...
public class Menu {
		...
		while (choix.equals("?"))
			try {
				...
				if (!choix.matches("[a-zA-Z\\s]*")) {
					choix = "?";
					System.out.println("Seuls les lettres de l'alphabet sont autoris�es");
				}
			} catch (InputMismatchException e) {
				System.err.println("ERREUR: Veuillez rentrer une cha�ne de caract�re valide !\n");
			}
		// scanner.close();
		return choix;
	}
	...
	public static Capacite choisirCapacite() {
		System.out.println("Choissisez votre type de capacit�\n1.\tRem�de\n2.\tBouclier\n3.\tSortil�ge\n4.\tEpee");
		int choix = Menu.choix();
		switch (choix) {
			case Capacite.REMEDE:
				return SauvegardeCapacite.chargerCapacite("Remede", Capacite.REMEDE);
			case Capacite.BOUCLIER:
				return SauvegardeCapacite.chargerCapacite("Bouclier", Capacite.BOUCLIER);
			case Capacite.SORTILEGE:
				return SauvegardeCapacite.chargerCapacite("Sortilege", Capacite.SORTILEGE);
			case Capacite.EPEE:
				return SauvegardeCapacite.chargerCapacite("Epee", Capacite.EPEE);
			default:
				return null;
		}
	}
